...
def bagOfWords(data, textField, target):
  d1, data = preprocessing(data, textField, target)
  labels = data.iloc[:, target]
  print(data)
...
def trainVid(file=None, target=None,resize=50):
    ...
    if file!=None:
        ...
        if not ext and ext not in compress_list and target==None:
            ...
            data,target=check_subfolder_data(file)
        data, cats=AutoFeatureSelection.image_processing(data,target,resize)

        print("cats",cats)

        return data,data.to_csv("media/pro_img.csv"),cats
    else:
        raise ValueError("{file} can't be null or empty")
...
def vidfile(request):
    if request.method=='POST':
        ...
        return redirect("/fileDownload?file1=/media/pro_img.csv&file2=/static/media/"+a.name+"_log.txt")
        # Return the response value
        # return response
    else:
        # Load the template
...
def extract(path, folder_name):
  with ZipFile(path, "r") as obj:
    ...
    print("FOLDER NAME: " + path.split(".")[0])
...
def audfile(request):
    if request.method=='POST':
        ...
        folder = (dataset.name.split('.')[0]).split(' ')[0]
